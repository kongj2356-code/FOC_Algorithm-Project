...
void AD_Init(void)
{
  ADC_InitTypeDef ADC_InitStructure;
    ...
    // 3. ADC时钟配置：ADCCLK = PCLK2 / 6 = 12MHz (72MHz/6)
    ...
    // 5. ADC通用配置
    ADC_InitStructure.ADC_Mode = ADC_Mode_InjecSimult;          // 注入同步模式
    ADC_InitStructure.ADC_ScanConvMode = DISABLE;               // 禁止扫描
    ADC_InitStructure.ADC_ContinuousConvMode = DISABLE;         // 禁止连续转换（由TIM触发）
    ADC_InitStructure.ADC_ExternalTrigConv = ADC_ExternalTrigConv_None; // 无规则组触发
    ADC_InitStructure.ADC_DataAlign = ADC_DataAlign_Right;      // 右对齐
    ADC_InitStructure.ADC_NbrOfChannel = 1;                     // 规则组通道数（此处不使用）
    ...
    // 11. 配置中断
    ADC_ITConfig(ADC1, ADC_IT_JEOC, ENABLE);  // 启用注入转换完成中断
...
void M1_Set_CurTorque(float Target_Current)
{
    ...
    foc.uq = PID_Controller(12.5f, 50.0f, 0.0f, err_q);
