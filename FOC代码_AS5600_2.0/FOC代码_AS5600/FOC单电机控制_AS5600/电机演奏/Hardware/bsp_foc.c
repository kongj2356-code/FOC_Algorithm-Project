...
static float foc_angle = 0.0f;
static uint32_t foc_last_us = 0;

/**
 * @brief  FOC 开环速度控制（用于发声）
...
void FOC_VelocityOpenLoop(float omega_e)
{
    ...
    // SysTick 72MHz，24bit 递减
    if (now < foc_last_us)
        Ts = (foc_last_us - now) / 72e6f;
    else
        Ts = (0xFFFFFF - now + foc_last_us) / 72e6f;

    if (Ts <= 0 || Ts > 0.02f) Ts = 1e-3f;

    foc_last_us = now;

    // 积分得到电角度
    foc_angle += omega_e * Ts;
    foc_angle = Angle_FOC(foc_angle);

    // 固定 Ud = 0, Uq 控制音量
    SetSVPWM_FOC(0.0f, 4.0f, foc_angle);
}
